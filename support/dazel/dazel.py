...
class DockerInstance:
    ...
    def send_command(self, args):
        ...
        docker_exec_command = "%s exec -i -e COLUMNS=%s -e LINES=%s -e TERM=%s %s %s %s" % (
            self.docker_command,
            term_size.columns,
            term_size.lines,
            os.environ.get("TERM", ""),
    ...
    def _add_volumes(self, volumes):
        ...
        if isinstance(volumes, str):
            ...
            raise RuntimeError(
                "DAZEL_VOLUMES must be comma-separated string " "or python iterable of strings"
            )
        ...
        if self.bazel_user_output_root:
            bazel_output_base = os.path.realpath(
                os.path.join(self.bazel_user_output_root, self.workspace_hex_digest)
            )
    ...
    def _add_ports(self, ports):
        ...
        if isinstance(ports, str):
            ...
            raise RuntimeError(
                "DAZEL_PORTS must be comma-separated string " "or python iterable of strings"
            )
    ...
    def _add_gpus(self, gpus):
        ...
        if isinstance(gpus, str):
            ...
            raise RuntimeError(
                "DAZEL_GPUS must be comma-separated string " "or python iterable of strings"
            )
    ...
    def _add_env_vars(self, env_vars):
        ...
        if isinstance(env_vars, str):
            ...
            raise RuntimeError(
                "DAZEL_ENV_VARS must be comma-separated string " "or python iterable of strings"
            )
    ...
    def _add_run_deps(self, run_deps):
        ...
        if isinstance(run_deps, str):
            ...
            raise RuntimeError(
                "DAZEL_RUN_DEPS must be comma-separated string " "or python iterable of strings"
            )
    ...
    def _find_workspace_directory(cls):
        ...
        while (
            directory
            and directory != root_dir
            and not os.path.exists(os.path.join(directory, BAZEL_WORKSPACE_FILE))
        ):
