...
class MotorSignalAgentTest : public testing::Test {
 protected:
  void SetUp() override {
    ...
    motor_characteristics_.no_load_speed    = 3400 *
                                              kRevPerMinToRevPerSec;
    motor_characteristics_.stall_torque.newtonmillimeters = 1.7 *
...
TEST_F(MotorSignalAgentTest, SendMotorSignalAndGetMotorTorque) {
  ...
  EXPECT_NEAR(motor_model_->getMotorTorque().newtonmillimeters,
              0.7 * kKilogramMmToNewtonMm,
              1e-3);
