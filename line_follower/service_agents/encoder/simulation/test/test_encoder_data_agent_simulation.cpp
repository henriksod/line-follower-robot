...
class EncoderDataAgentTest : public testing::Test {
 protected:
  void SetUp() override {
    ...
    auto encoder_model{ std::make_unique<EncoderModel>(
                          encoder_characteristics_) };
...
TEST_F(EncoderDataAgentTest, GetEncoderData) {
  ...
  encoder_data_consumer_agent.onReceiveData(
    [&] (EncoderData const& encoder_data) {
        EXPECT_EQ(encoder_data.revolutions_per_second,
  ...
  EXPECT_NEAR(received_encoder_data.step_position,
              floor(estimated_smooth_step),
              1);
}
}  // namespace
}  // namespace line_follower
