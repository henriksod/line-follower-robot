...
constexpr double kLedArraySpacingMeters{ kLedArraySpacingMilli *
...
std::vector<TrackLineSegment> input_track_line_list = {
  { { { kLeftmostLedPositionX,  1.0,
    0.0                                                          },
    { kLeftmostLedPositionX,  -1.0,                   0.0  } },
    kLedArraySpacingMeters / 3.0,
    0U, true  },
  { { { kRightmostLedPositionX, 1.0,
    0.0                                                          },
    { kRightmostLedPositionX, -1.0,
      0.0                     } },
    kLedArraySpacingMeters / 3.0, 0U, true  },
  { { { kMiddleLedPositionX,    1.0,
    0.0                                                          },
    { kMiddleLedPositionX,    -1.0,
      0.0                                        } },
    kLedArraySpacingMeters / 3.0, 0U,
    true  },
  { { { 1.0,                    1.0,
    0.0                                                          },
    { 1.0,                    1.0,
  ...
  { { { kMiddleLedPositionX,    1.0,
    ...
    { kMiddleLedPositionX,    -1.0,
      0.0                                                                              } },
    kLedArraySpacingMeters / 3.0, 0U, false },
  { { { kLeftmostLedPositionX,  0.0,
    0.0                                                                                                 },
    { kRightmostLedPositionX, 0.0,
      0.0                                                                                  } },
    kLedArraySpacingMeters / 3.0, 0U, true  },
  { { { kMiddleLedPositionX,    1.0,
    0.0                                                                                         },
    { kMiddleLedPositionX, -1.0,
      0.0                                                                                     } },
    kLedArraySpacingMeters * 2.5, 0U, true  } };
std::vector<std::array<IrSensorData,
                       kMaxIrSensorArrayNumberOfLeds> >
expected_activated_leds_list = {
  ...
  { black, black, black, black, black, black, black, black, black, black,
    ...
    black, black, black,
    black   },
  { white, white, white, white, white, white, black, black, black, white,
    white,
    white, white, white,
    white   }
};
 ...
 protected:
  void SetUp() override {
    ...
    auto ir_sensor_array_model{ std::make_unique<IrSensorArrayModel>(
                                  ir_sensor_array_characteristics_) };
    ir_sensor_array_model_               = ir_sensor_array_model.get();
    ir_sensor_array_data_producer_agent_ =
      std::make_unique<IrSensorArrayDataProducerAgent>(std::move(
                                                         ir_sensor_array_model));
  ...
  std::unique_ptr<IrSensorArrayDataProducerAgent>
  ir_sensor_array_data_producer_agent_;
...
TEST_F(IrSensorArrayDataAgentTest, GetIrSensorArrayData) {
  ...
  while (iter1 != input_track_line_list.end() &&
         iter2 != expected_activated_leds_list.end()) {
    ...
    ir_sensor_array_model_->setTrackLines(track_segment,
                                          ir_sensor_array_pose_in_track_segment);

    bool receiveDataWasCalled{ false };
    IrSensorArrayDataConsumerAgent ir_sensor_array_data_consumer_agent{};
    ir_sensor_array_data_consumer_agent.onReceiveData([&] (IrSensorArrayData
                                                           const&
                                                           ir_sensor_array_data)
        {
          for (std::size_t idx{ 0U }; idx < kNumberOfLeds; ++idx) {
            EXPECT_EQ(ir_sensor_array_data.ir_sensor_readings[idx].
                      detected_white_surface,
    ...
    ir_sensor_array_data_consumer_agent.attach(
      *ir_sensor_array_data_producer_agent_);

    ir_sensor_array_data_producer_agent_->schedule(scheduler_,
                                                   kUpdateIntervalMicros);
    ...
    for (std::size_t idx{ 0U }; idx < kNumberOfLeds; ++idx) {
      std::cerr <<
        received_ir_sensor_array_data.ir_sensor_readings[idx].
        detected_white_surface
                <<
        " (" << expected_output[idx].detected_white_surface << ") ";
    }
