...
class SchedulerProducerAgent::Impl final {
 ...
 public:
  Impl()
    : time_agent_{std::make_unique<TimeAgent>()} {}
...
void SchedulerProducerAgent::registerScheduler(
  SchedulerConsumerAgent& consumer_agent,
  uint32_t                time_interval_us) {
...
void SchedulerProducerAgent::deregisterScheduler(
  SchedulerConsumerAgent& consumer_agent) {
