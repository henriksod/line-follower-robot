...
class SchedulerProducerAgent final : ProducerAgent<UniqueID>
{
 public:
   SchedulerProducerAgent();
    ...
    /// @param consumer_agent The consumer agent to send a tick to at the set time interval
...
/// A schedule consumer agent. Will only work if registered towards a scheduler.
...
class SchedulerConsumerAgent final : public ConsumerAgent<UniqueID>
{
 public:
   ...
   Maybe<UniqueID> getUniqueID()
   {
      return maybe_unique_id_;
   }

   /// @brief Set the unique ID of the consumer agent
   /// @param unique_id The unique ID to set for this consumer agent
   /// @param tag This function can only be called by a scheduler producer agent
