                                       ...
                                       Pose const& ir_sensor_array_pose_in_track_segment)
{
    ...
    for (std::size_t idx{0U}; idx < ir_array_characteristics_.number_of_leds; ++idx)
    {
        ...
        // Get the led position relative to the origin, which is the middle of the ir array
        Vector3<double> led_position_rel_origin{(led_idx - middle_led_idx) * array_spacing, 0.0, 0.0};
        ...
        led_line.set(ir_sensor_array_position + rotated(
                        ...
                        Vector3<double>{led_position_rel_origin[0] - array_spacing / 2.0, 0.0, 0.0}),
        ...
        for (auto& track_line_segment : track_segment.track_lines)
        {
            ...
            Line<double> orthogonal_led_line{led_line.center() + orthogonal_led_line_vector * array_spacing / 2.0,
                                             led_line.center() - orthogonal_led_line_vector * array_spacing / 2.0};

            Line<double> const track_line{convert(track_line_segment.line)};
            auto const track_line_strip{geometry::sweepAlongWidth(track_line, track_line_segment.width)};
            bool any_overlap{
            ...
            if (any_overlap)
            {
                ...
                /// TODO: Calculate digital reading based on distance from track line edge wrt thickness
