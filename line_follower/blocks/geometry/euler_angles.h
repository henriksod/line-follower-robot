...
class EulerAngles {
 public:
  ...
  EulerAngles(T roll = 0.0, T pitch = 0.0, T yaw = 0.0)
    : roll_{roll}
    , pitch_{pitch}
    , yaw_{yaw} {}

  template<typename T1>
  EulerAngles(const EulerAngles<T1>& other)
    : roll_{other.roll()}
    , pitch_{other.pitch()}
    , yaw_{other.yaw()} {}
  ...
  bool is_zero(T1 eps = 0) const {
    return is_scalar_zero(roll_, eps)
           && is_scalar_zero(pitch_, eps)
           && is_scalar_zero(yaw_, eps);
  }

  bool is_nan() const {
    return std::isnan(roll_) || std::isnan(pitch_) || std::isnan(yaw_);
  }

  bool is_inf() const {
    return std::isinf(roll_) || std::isinf(pitch_) || std::isinf(yaw_);
  }

  bool is_finite() const {
    return std::isfinite(roll_) && std::isfinite(pitch_) &&
           std::isfinite(yaw_);
  }
