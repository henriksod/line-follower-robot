...
class Line {
 public:
    ...
    explicit Line(Vector3<T> const& v) {
        from_ = Vector3<T>{0.0, 0.0, 0.0};
        to_ = v;
    }
    ...
    const Line<T> operator-() { return Line<T>(to_, from_); }

    bool operator==(const Line<T>& line) { return from_ == line.from() && to_ == line.to(); }

    bool operator!=(const Line<T>& line) { return !(*this == line); }
