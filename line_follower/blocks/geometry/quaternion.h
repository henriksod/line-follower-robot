...
class Quaternion {
 public:
  ...
  Quaternion(T w = 1.0, T x = 0.0, T y = 0.0, T z = 0.0)
    : w_{w}, x_{x}, y_{y}, z_{z} {}
  ...
  template<typename T1>
  Quaternion(const Vector3<T1>& vector)
    : w_{static_cast<T1>(0.0)}, x_{vector.x()}, y_{vector.y()},
    z_{vector.z()} {}
  ...
  T complex_norm_squared() const {
    return x_ * x_ + y_ * y_ + z_ * z_;
  }

  template<typename T1 = T>
  bool is_zero(T1 eps = 0) const {
    return is_scalar_zero(w_, eps)
           && is_scalar_zero(x_, eps)
           && is_scalar_zero(y_, eps)
  ...
  bool is_nan() const {
    return std::isnan(w_) || std::isnan(x_) || std::isnan(y_) ||
           std::isnan(z_);
  }

  bool is_inf() const {
    return std::isinf(w_) || std::isinf(x_) || std::isinf(y_) ||
           std::isinf(z_);
  }

  bool is_finite() const {
    return std::isfinite(w_) && std::isfinite(x_) && std::isfinite(y_) &&
           std::isfinite(z_);
  }
...
inline EulerAngles<T>to_euler(Quaternion<T>const& x, T eps = 1e-12) {
  ...
  return { { atan2(2 * (x.w() * x.y() - x.x() * x.z()),
                   1 - 2 * (x.y() * x.y() + x.z() * x.z())),
    std::asin(2 * v),
    atan2(2 * (x.w() * x.x() - x.y() * x.z()),
          1 - 2 * (x.x() * x.x() + x.z() * x.z())) } };
}
...
inline T norm_l1(const Quaternion<T>& x) {
  return std::abs(x.w()) + std::abs(x.x()) + std::abs(x.y()) +
         std::abs(x.z());
}

template<typename T, typename T1>
inline T norm_lk(const Quaternion<T>& x, T1 k) {
  return std::pow(std::pow(std::abs(x.w()), k)
                  + std::pow(std::abs(x.x()), k)
                  + std::pow(std::abs(x.y()), k)
                  + std::pow(std::abs(x.z()), k), 1.0 / k);
}
...
inline bool operator==(const Quaternion<T1>& x, const Quaternion<T2>& other) {
  return x.w() == other.w() && x.x() == other.x() && x.y() == other.y() &&
         x.z() == other.z();
}
...
inline bool nearly_equal(const Quaternion<T1>& x,
                         const Quaternion<T2>& other,
                         T3                    eps) {
  return is_nearly_equal(x.w(), other.w(), eps)
         && is_nearly_equal(x.x(), other.x(), eps)
         && is_nearly_equal(x.y(), other.y(), eps)
         && is_nearly_equal(x.z(), other.z(), eps);
}
...
inline T dot(const Quaternion<T>& x, const Quaternion<T>& other) {
  return x.w() * other.w() + x.x() * other.x() + x.y() * other.y() + x.z() *
         other.z();
}
...
inline Quaternion<T>cross(const Quaternion<T>& x, const Quaternion<T>& other) {
  return { 0,
           x.y() * other.z() - x.z() * other.y(),
           x.z() * other.x() - x.x() * other.z(),
