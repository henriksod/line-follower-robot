...
class FunctionObject<Result(Args...)> {
 ...
 public:
    ...
    template <typename Func>
    explicit FunctionObject(const Func& x)
        : f(new detail::concrete_function<typename detail::func_filter<Func>::type, Result,
                                          Args...>(x)) {}

    explicit FunctionObject(const FunctionObject& rhs) : f(rhs.f ? rhs.f->clone() : nullptr) {}
    ...
    FunctionObject& operator=(const Func& x) {
        auto* temp = new detail::concrete_function<typename detail::func_filter<Func>::type, Result,
                                                   Args...>(x);
